...
def postprocess_args(args):
    ...
    if not args.augment:
        save_path2 += '-NoAug'
    if args.sa or args.saa:
        save_path2 += '-sa'
    # if args.saa:
    #     save_path2 += '-saa'
    save_path2 += '-epoch{}-p{:02g}'.format(args.max_epoch, args.p)
    if args.train_aug:
        save_path2 += '-train_aug'
    save_path1 = os.path.join(args.save_dir, save_path1)
    if args.fine_tune:
...
def get_command_line_parser():
    ...
    parser.add_argument('--saa', action="store_true", default=False)
    ...
    parser.add_argument('--fix_batch', action="store_true", default=False)
...
def read_and_parse_file(file_path):
    ...
    with open(file_path, 'r') as file:
        data = json.load(file)  # 假设文件内容是JSON格式
