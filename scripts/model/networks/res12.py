...
class ResNet(nn.Module):
    ...
    def __init__(self, block=BasicBlock, keep_prob=1.0, avg_pool=True, drop_rate=0.1, dropblock_size=5):
        ...
        self.drop_rate = drop_rate
        self.new = nn.Sequential(nn.Conv2d(320, 640, kernel_size=3, stride=1, padding=1, bias=False), nn.BatchNorm2d(640), nn.LeakyReLU(0.1), nn.MaxPool2d(2))
    ...
    def forward(self, x):
        # [100, 3, 84, 84]
        x = self.layer1(x)  # [100, 64, 42, 42]
        x = self.layer2(x)  # [100, 160, 21, 21]
        x = self.layer3(x)  # [100, 320, 10, 10]
        x = self.layer4(x)  # [100, 640, 5, 5]
        # x = self.layer4(x) * 0.7 + self.new(x) * 0.3
        if self.keep_avg_pool:
            self.avgpool = nn.AvgPool2d(x.shape[-1], stride=1)
            x = self.avgpool(x)
        x = x.view(x.size(0), -1)
        return x
