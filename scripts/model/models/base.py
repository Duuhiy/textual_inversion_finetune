...
from model.networks.spatial_attention import SpatialAttention
    ...
    def split_instances(self, data):
        ...
        if self.training:
            return  (torch.Tensor(np.arange(args.way*args.shot)).long().view(1, args.shot, args.way), 
                     torch.Tensor(np.arange(args.way*args.shot, args.way * args.shot + args.query)).long().view(1, args.query, 1))
        else:
            return  (torch.Tensor(np.arange(args.eval_way*args.eval_shot)).long().view(1, args.eval_shot, args.eval_way), 
                     torch.Tensor(np.arange(args.eval_way*args.eval_shot, args.eval_way*args.eval_shot + args.eval_query)).long().view(1, args.eval_query, 1))
    ...
    def forward(self, x, get_feature=False):
        if get_feature:
            ...
            if self.args.sa:
                sa_w = self.sa(x)
                x_sa = sa_w * 0.1 + x
                instance_embs = self.encoder(x_sa)
                instance_embs = instance_embs / instance_embs.norm(dim=-1, keepdim=True)
            else:
